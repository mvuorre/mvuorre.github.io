...
Given this parameterization, the intercept of the model ($\beta_0$) is going to be the standardized false alarm rate (probability of saying 1 when predictor is 0), which we take as our criterion *c*. The slope of the model is the increase in the probability of saying 1 when the predictor is 1, in *z*-scores, which is another way of saying *d'*. Therefore, $c = -zFAR = -\beta_0$, and $d' = \beta_1$. 

**Note. This parameterisation may differ from more common bias estimates. I received [two very helpful issues on this on GitHub](https://github.com/mvuorre/mvuorre.github.io/issues/1) (thanks!).** 

>Additionally, I suggest to add a paragraph, how to compute different bias estimate. Macmillian and Creelman are using c=-0.5(zHR+zFAR). This should be easily computed from posterior distribution as c = -0.5(2β0 - β1). this could help others to transit from nonhierarchical SDT to SDT in brms (<https://github.com/fidadoma>)

>The c used throughout Matt’s page is not typical and likely to be very misleading. McMillan & Creelman use the c calculation noted above, and DeCarlo note in their appendix that this corresponds to using effect coding (better yet, -.5 vs. +.5 coding). I have never encountered c being calculated the way Matt does or that DeCarlo mentions early in his paper. Psych has settled on calculating bias as -.5*(zHR+zFAR) or, sometimes, the Beta or slope measure. I’ve never encountered c=-zFAR. (<https://github.com/MikeKSU>)

**Keep these useful comments in mind when reading.**
...
```{r densityplot, echo = F, fig.cap = "The (approximate) joint posterior density of subject 53's SDT parameters. Lighter yellow colors indicate higher posterior density. The red dot indicates the 'manually' calculated MLE point estimate of d'.", preview = TRUE}
...
The results are very similar, but note that priors were included only in the nonlinear syntax model. The only real difference is that the MCMC algorithm explored `evsdt_2`'s posterior less efficiently, as shown by the smaller effective sample sizes (`..._ESS`) for both parameters. This means that the random draws from the posterior distribution, for `evsdt_2`, have greater autocorrelation, and therefore we should possibly draw more samples for more accurate inference.  The posterior distributions obtained with the 2 methods are shown in Figure \@ref(fig:densityplot2).
...
These mean effects are visualized as a colored density in the left panel of Figure \@ref(fig:evsdt-glmm-viz1). However, the GLMM also returns estimates of the parameters' (co)variation in the population. Notice that we also calculated the sample standard deviations, which also provide this information, but we have no estimates of uncertainty in those point estimates. The GLMM, on the other hand, provides full posterior distributions for these parameters.

The heterogeneity parameters are reported in the "Group-Level Effects"[^2] table, above. We find that the criteria are positively correlated with *d'*s (recall that Intercept = -*c*). The two standard deviations are visualized in the right panel of Figure \@ref(fig:evsdt-glmm-viz1).
...
It is evident in Figure \@ref(fig:evsdt-glmm-viz1) that the sample means approximately match the posterior mode, but less so for the sample standard deviations, which are far from the peak of the standard deviations' posterior distribution. By ignoring the uncertainty in the subject-specific parameters, the 'manual calculation' method has over-estimated the heterogeneity of *d'*s and *c*s in the population, in comparison to the GLMM which takes the subject-specific parameters' uncertainty into account.
...
This "partial pooling" of information [@gelman_data_2007] is evident when we plot the GLMM's subject-specific parameters in the same scatterplot with the N models method (calculating point estimates separately for everybody; Figure \@ref(fig:evsdt-glmm-viz2)).
...
slopes <- coef(evsdt_glmm)$subno[, , "isold"] %>%
  as.data.frame() %>%
  rownames_to_column("subno") %>%
